import { NavLink, Flex, Rating, Text, Button } from "@mantine/core";
...
export function ChatSummary({summary}: SummaryProp){
    ...
    const retrieveSummary = () => {
        if (chatResponse.summary.length == 0){
     ...
     return(
        <>
            <NavLink label={<Text fw={500}>Star Rating and Summary</Text>} onClick={() => retrieveSummary()} active variant='light' color="pink" className="rounded-2xl">
