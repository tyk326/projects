...
export const THEME_CONFIG: Record<ThemeStyle, {
  ...
  'lofi': {
    prompt: 'Lofi hip hop aesthetic, chill nostalgic vibes, muted pastel color palette, soft gradients, cozy warm atmosphere, retro 90s anime style, StudyGirl aesthetic, maintain original subjects and scene composition',
    model: 'black-forest-labs/flux-dev',
    strength: 0.62,
    guidance: 5.0,
  },
...
export async function generateImage(
  ...
  // Calculate correct DPI for landscape
  // For canvas-12x16 in landscape = 16" wide × 12" tall
  const [smallDim, largeDim] = (canvasSize || 'canvas-16x12')
    .replace('canvas-', '')
    .split('x')
    .map(Number);
  const canvasWidthInches = Math.max(smallDim, largeDim); // Larger dimension = width in landscape
  const calculatedDPI = Math.round(dimensions.width / canvasWidthInches);

  console.log('🎨 Generating HIGH-RES LANDSCAPE image:', {
    theme,
    canvasSize: canvasSize || 'default (16x12)',
    canvasPhysicalSize: `${canvasWidthInches}" wide × ${Math.min(smallDim, largeDim)}" tall`,
    dimensions: `${dimensions.width}×${dimensions.height} pixels`,
    aspectRatio: dimensions.aspectRatio,
    orientation: dimensions.orientation,
    targetDPI: calculatedDPI,
  });
  ...
  try {
    ...
    if (Array.isArray(output) && output.length > 0) {
      ...
      const [dim1, dim2] = canvasSize?.replace('canvas-', '').split('x').map(Number) || [12, 16];
      const widthInches = Math.max(dim1, dim2);
      const dpi = Math.round(dimensions.width / widthInches);
      console.log(`   DPI: ~${dpi} (for print)`);
      return output[0];
    }
